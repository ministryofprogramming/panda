...
// List maps a Panda command to a slice of Command
type List map[string]Commands

// Available plugins for the `panda` command.
// It will look in $GOPATH/bin and the `./plugins` directory.
// This can be changed by setting the $PANDA_PLUGIN_PATH
// environment variable.
//
// Requirements:
// * file/command must be executable
// * file/command must start with `panda-`
...
func Available() (List, error) {
	...
	from, err := env.MustGet("PANDA_PLUGIN_PATH")
	if err != nil {
	...
	for _, p := range paths {
		...
		err := filepath.Walk(p, func(path string, info os.FileInfo, err error) error {
			...
			if strings.HasPrefix(base, "panda-") {
